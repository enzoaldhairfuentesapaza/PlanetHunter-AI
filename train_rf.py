...
# 2. Filtrar solo las clases que nos interesan y crear target
df = df[df['koi_disposition'].isin(['CONFIRMED', 'FALSE POSITIVE'])]
df['target'] = df['koi_disposition'].map({'CONFIRMED': 1, 'FALSE POSITIVE': 0})

# 3. Preparar features y target
# Eliminamos columnas que no aportan y posibles strings
X = df.drop(columns=['kepid', 'rowid', 'koi_disposition', 'target'], errors="ignore")
y = df['target']

# Convertir todo a float para evitar errores
X = X.apply(pd.to_numeric, errors='coerce')  # convierte strings a NaN
X = X.fillna(0)  # opcional: rellenar NaN con 0, puedes cambiar por otro valor o estrategia
